...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  menus.forEach(m => el.menu.insertAdjacentHTML('beforeend',
...
el.form.addEventListener('submit', async e => {
  ...
  if (el.imgIn.files.length) {
    ...
    const { publicURL } = supabase.storage.from('user-images').getPublicUrl(fname);
    imageUrl = publicURL;
  }
...
function updateUI() {
  ...
  const menuId = +el.menu.value, posted = (all[today]||[]).includes(menuId);
  ...
  if (cnt > 0) {
    ...
    el.btns.querySelectorAll('button').forEach(b=>
      b.addEventListener('click',()=>document.getElementById(b.dataset.key)
